    ...
    for j := 0; j < rounds; j++{
        for idx := 0; idx < len(monkeys); idx++{
            for len(monkeys[idx].items) != 0{
                (&monkeys[idx]).do_op()
                monkeys[idx].items[0] /= 3
                is_t := 0
                if monkeys[idx].items[0] % monkeys[idx].test == 0{
                    is_t = 1
                }
                monkeys[monkeys[idx].if_cond[is_t]].items = append(monkeys[monkeys[idx].if_cond[is_t]].items, monkeys[idx].items[0])
                monkeys[idx].items = append([]int{}, monkeys[idx].items[1:]...)
                fmt.Println(monkeys[idx].items)
                monkeys[idx].count ++ 
            }
        }
    }
